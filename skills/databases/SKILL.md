...
3. Create subscription on target: `CREATE SUBSCRIPTION upgrade_sub CONNECTION 'host=target-db.internal dbname=app user=repl' PUBLICATION upgrade_pub;` - omit `password=` so libpq reads the subscriber's mode-0600 `~/.pgpass`; an inline password is stored in `pg_subscription` and appears in catalog dumps and logs

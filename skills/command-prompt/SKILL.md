...
name: command-prompt
description: >
  Deprecated name. Use shell-scripting; load this notice only when command-prompt is explicitly requested.
license: MIT
metadata:
...
Only when the user explicitly invokes the old **command-prompt** name or asks how to migrate it.
...
1. Explain that this skill was renamed or merged into **shell-scripting**.
2. If **shell-scripting** is installed, offer it by name and use its own instructions when the request authorizes that work.

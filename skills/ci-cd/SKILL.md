...
- [ ] **Version pinning on tools**: specific versions over floating aliases - `node:22`, not `node:lts`; `python:3.13`, not `python:3`. Specific versions prevent silent breakage, then pin the resulting image by digest per the SHA pinning item above, e.g. `node:22@sha256:<digest>`.

...
name: cluster-health
description: >
  Deprecated name. Use kubernetes-health; load this notice only when cluster-health is explicitly requested.
license: MIT
metadata:
...
Only when the user explicitly invokes the old **cluster-health** name or asks how to migrate it.
...
1. Explain that this skill was renamed or merged into **kubernetes-health**.
2. If **kubernetes-health** is installed, offer it by name and use its own instructions when the request authorizes that work.

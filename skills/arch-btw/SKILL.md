...
name: arch-btw
description: >
  Deprecated name. Use arch-linux; load this notice only when arch-btw is explicitly requested.
license: MIT
metadata:
...
Only when the user explicitly invokes the old **arch-btw** name or asks how to migrate it.
...
1. Explain that this skill was renamed or merged into **arch-linux**.
2. If **arch-linux** is installed, offer it by name and use its own instructions when the request authorizes that work.

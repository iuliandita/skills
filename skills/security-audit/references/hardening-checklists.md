...
### Active package compromise triage

Use this when a public package compromise lands and the user asks whether repos are
affected. Keep it repo-only unless the user asks for live infra or token rotation work.

1. Verify the incident from primary sources first: upstream security advisory, package
   registry quarantine/yank notice, and reputable security writeups for IOCs.
...
3. Search all repo manifests and lockfiles, not only source imports:
   - Python: `pyproject.toml`, `requirements*.txt`, `setup.py`, `setup.cfg`,
     `poetry.lock`, `uv.lock`, `Pipfile.lock`, conda env files
...
4. Search source for runtime imports/usages that may pull dynamic dependencies:
   `import <package>`, `from <package>`, and framework-specific import aliases.
5. Search for campaign IOCs separately from package names: payload files/directories,
   injected lifecycle hooks, suspicious branch names, unusual commit authors/emails, and
   newly created mass branches after disclosure time.
6. Check local runtime environments only if relevant and low-risk, using non-importing
   checks first:
   - `python3 -m pip show <pkg>`
   - `python3 - <<'PY'` with `importlib.util.find_spec()` instead of importing the
     suspect package
7. If GitHub propagation is part of the campaign, check remote repos updated since the
   incident for branch counts, suspicious branch names, recent commits by known malicious
   authors/emails, or IOC strings.
8. Report in three buckets: direct vulnerable dependency found, indicators of compromise
   found, and clean/no evidence found with scope and caveats.

If any direct hit exists, advise treating that environment as compromised: rotate secrets,
rebuild from clean state, review logs, and pin to known clean versions. If only unrelated
string matches appear, call them out explicitly as false positives.

#### Dated example: April 2026 Mini Shai-Hulud

Public reporting around the April 2026 campaign described PyPI `lightning==2.6.2` and
`lightning==2.6.3`, plus npm `intercom-client@7.0.4`, as affected versions. Treat this
as an example and verify current advisories before acting. Reported clean pins included
`lightning==2.6.1` and `intercom-client@7.0.3`.

Reported execution triggers:
...
- GitHub propagation using stolen tokens, including many branches per writable repo

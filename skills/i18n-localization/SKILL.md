...
description: >
  App i18n only, not standalone translation: localize UI catalogs, validate placeholders, and audit missing locale keys.
license: MIT
...
- Reviewing code quality or style issues - use **code-review** or **code-simplification**
- Building AI/LLM features that produce multilingual output at runtime - use **llm-app-development**
...
- **Skill name:** I18N-LOCALIZATION
- **Deliverable bucket:** `audits`
- **Mode:** conditional. When invoked to **audit, review, or improve** existing i18n (hardcoded-string audit, catalog completeness check, translation quality review), apply the reporting size and evidence rules in `references/output-contract.md` and write the deliverable to `docs/local/audits/i18n-localization/<YYYY-MM-DD>-<slug>.md`. When invoked to **set up i18n from scratch, generate translations for new keys, or answer a question**, respond freely without the contract.
- **Deliverable path:** `docs/local/audits/i18n-localization/<YYYY-MM-DD>-<slug>.md`
...
- **llm-app-development** - when AI-generated text needs to respond in the user's language at runtime,
  the AI locale context is an i18n concern. The response quality is an llm-app-development concern.

...
name: nixos-btw
description: >
  Deprecated name. Use nixos; load this notice only when nixos-btw is explicitly requested.
license: MIT
metadata:
...
Only when the user explicitly invokes the old **nixos-btw** name or asks how to migrate it.
...
1. Explain that this skill was renamed or merged into **nixos**.
2. If **nixos** is installed, offer it by name and use its own instructions when the request authorizes that work.
